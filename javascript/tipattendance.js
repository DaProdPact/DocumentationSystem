$(document).ready(function () {
  ...
  $('#tipattendanceprint').hide();
  $('#tipattendanceprintdetails').hide();
  ...
  $('#dropdown_logout').on('click',function(){
    ...
    $('#tipattendanceprintdetails').show();
    $('#tipattendanceprint').hide();
      window.print();
 ...
 success: function (res) {
   ...
   for (user of users) {
     ...
     rows += `<tr class="table-active" style="height: 15px; font-size:11px; font-weight:bold;">
         ...
         <td>${user.Barangay} ${user.Municipality} ${user.Permanent_Province}</td>

         <td>${user.Contact_Number}</td>
    


         <td> <i value="view" class="fas fa-eye text-success p-0 view_data" name="view" id="${user.id} "></i>
         <i value="view" class="fas fa-edit text-primary p-0 update_data" name="view" id="${user.id} "></i>
         <i value="view" class="fas fa-trash text-danger p-0 delete_data" name="view" id="${user.id} "></i>
               </td>
     </tr>`;
   }
