    ...
    // })
      ...
      var firstname = '';
        ...
        $.ajax({ 
         ...
         url: "adminbatchaction.php?page=" + page,
         method: "GET",
         data:{
          getbatch:getbatch,
          trid:trid,
         ...
         success: function (res) {
           ...
           for (request of requests){
            var startdate = my_date_format(request.date_start);
            var dateend = my_date_format(request.date_end);
        
             rows += `<tr class="table-active" style="height: 15px; font-size:11px; font-weight:bold;">
                  <td>${request.batch_name}</td> 
                  <td>${request.batch_year}</td> 
                  <td>${startdate}</td> 
                  <td>${dateend}</td>`
                  if(request.batch_validation == '2'){
           ...
           $('.view_data').on('click',function(){
            ...
            $.ajax({
              ...
              success:function(response){
                  window.location.href ='http://localhost/DocumentationSystem/admin/admintraineeslist.php'  
              }
            })
           })
              ...
              >
               ...
               <div class="row">
                ...
                <label class="ms-1" for="batchyear">Date End</label>
                   <input type="date" class="form-control" id="date_end" placeholder="Date End" value="${edit.date_end}">
                 </div>
                  ...
                  if(edit.batch_validation == '2'){
                    rows += `<select class="form-select form-select-sm py-2 border-2 border-secondary" id="batch_verify" >
                ...
                $('.update_detail').on('click',function(){
                  ...
                  var batch_verify = $('#batch_verify').val()
